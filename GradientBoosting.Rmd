...
print(trainingSet)
trainingfeatures <- subset(trainingSet, select=-c(SalePrice, SalePrice_log))
# Get the labels of the training set
traininglabels <- trainingSet$SalePrice_log
# Get the features of the testing set
testingfeatures <- subset(testingSet, select = -c(SalePrice, SalePrice_log))
```
...
# train GBM model
gbm.fit <- gbm(
  formula = traininglabels ~ . - (SalePrice+SalePrice_log+Id+Utilities),
  data = trainingSet,
...
error = expm1(prediction) - (testingSet$SalePrice)
mse = mean(error^2)
print(sqrt(mse))
```
...
prediction2 <- predict(gbm.fit, test_filtered)
error2 = expm1(prediction2)-res_data$SalePrice
